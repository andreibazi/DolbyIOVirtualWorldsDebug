...
#include <dolbyio/comms/unreachable.h>
...
template <typename CallableType,
          typename CallableRet,
          typename PrevSolverTraits>
    ...
    try {
      ...
      if constexpr (ret_type_helper::wraps == wrap_async_type::value) {
        if constexpr (std::is_same_v<void, CallableRet>) {
          ...
          return typename ret_type_helper::async_result_type{
              callable(), async_result_tags::resolved};
        }
      }
    } catch (...) {
      return typename ret_type_helper::async_result_type{
          std::current_exception(), async_result_tags::failed};
    }
  }
};
...
struct solver_synchronisation_guard : public traits::lock_guard_type {
 public:
  solver_synchronisation_guard(typename traits::lock_type& self)
      : traits::lock_guard_type(self) {}
};
...
class DLB_COMMS_TEMPLATE_EXPORT solver_base : public traits::lock_type {
 public:
  using err_cb_type =
      typename traits::template callback_type<void(std::exception_ptr&&)>;
...
class DLB_COMMS_TEMPLATE_EXPORT low_level_solver_chain {
 public:
  void set_chained_solver(
      const std::shared_ptr<low_level_solver_chain<traits>>& slv) {
    assert(!chained_solver_);
    chained_solver_ = slv;
  }

  const std::shared_ptr<low_level_solver_chain<traits>> get_chained_solver()
      const {
    assert(chained_solver_);
...
template <typename T, typename traits>
class DLB_COMMS_TEMPLATE_EXPORT low_level_solver
    : public low_level_solver_chain<traits> {
 ...
 public:
  ...
  void set_callbacks_from(low_level_solver<T, traits>* other) {
    ...
    assert(!cb_);
    assert(this->get_chained_solver() &&
           other == this->get_chained_solver().get());
    cb_ = solver_lambda_helpers<T, traits>::chain_value(other);
    bool has_error = base_.set_propagate_error_callback(
        solver_lambda_helpers<T, traits>::chain_error(other));
   ...
   */
  bool set_error_callback(err_cb_type&& cb) {
    return base_.set_error_callback(std::move(cb));
  }
 ...
 */
template <typename traits>
class DLB_COMMS_TEMPLATE_EXPORT low_level_solver<void, traits>
    : public low_level_solver_chain<traits> {
  solver_base<traits> base_{};
 ...
 public:
  ...
  void set_callbacks_from(low_level_solver<void, traits>* other) {
    ...
    bool has_error = base_.set_propagate_error_callback(
        solver_lambda_helpers<void, traits>::chain_error(other));
...
class [[nodiscard]] solver {
 ...
 public:
   ...
   */
  solver(low_level_solver_ptr<T, traits>&& slv) noexcept
      : slv_(std::move(slv)) {
    assert(slv_);
  }
 ...
 private:
  void fail_if_not_destroyed() {
    auto slv = std::move(slv_);
    if (slv) {
      slv->fail(std::make_exception_ptr(
          dolbyio::comms::async_operation_canceled("Destroyed")));
    }
  }
...
class [[nodiscard]] async_result_with_solver {
 public:
  ...
  async_result_with_solver()
      : solver_(std::make_shared<class low_level_solver<T, traits>>(
            async_result_tags::intentional_usage)),
...
/// @cond DO_NOT_DOCUMENT
// Helper type to implement operations which do not necessrily need access to
// async_result's private parts, but can be expressed using the core
// async_result API:
template <typename traits>
struct async_result_helper {
  template <typename U>
  static async_result<void, traits> discard_result(
      async_result<U, traits>&& res) {
  ...
  static async_result<std::exception_ptr, traits> promote_error_to_async(
      async_result<void, traits>&& res) {
    return std::move(res)
        .then([]() -> std::exception_ptr { return {}; })
        .consume_errors([](std::exception_ptr&& e) -> std::exception_ptr {
          return std::move(e);
        });
  }

  template <typename U>
  static async_result<std::exception_ptr, traits> discard_result_and_get_error(
      async_result<U, traits>&& res) {
...
template <bool optimize_for_size, typename T, typename traits>
struct result_state_type {};

template <typename T, typename traits>
struct result_state_type<false, T, traits> {
  using type = std::variant<
      std::monostate,                   // async_result_holds::nothing
  ...
  static size_t holds(const type& state) { return state.index(); }
  static void reset(type& state) {
    state.template emplace<async_result_holds::nothing>();
  }

  static low_level_solver_ptr<T, traits> extract_solver(type& state) {
    assert(holds(state) == async_result_holds::solver);
    low_level_solver_ptr<T, traits> my_solver =
        std::move(std::get<async_result_holds::solver>(state));
    reset(state);
    return my_solver;
  }

  static typename variant_value<T>::type extract_value(type& state) {
    assert(holds(state) == async_result_holds::value);
    typename variant_value<T>::type val =
        std::move(std::get<async_result_holds::value>(state));
    reset(state);
    return val;
  }

  static std::exception_ptr extract_error(type& state) {
    assert(holds(state) == async_result_holds::error);
    std::exception_ptr e =
        std::move(std::get<async_result_holds::error>(state));
    reset(state);
    assert(e);
    return e;
  }
...
template <typename T, typename traits>
struct result_state_type<true, T, traits> {
  using type = low_level_solver_ptr<T, traits>;
  static size_t holds(const type& s) {
    return s ? async_result_holds::solver : async_result_holds::nothing;
  }

  static void reset(type& state) { state.reset(); }
  ...
  static typename variant_value<T>::type extract_value(type&) {
    DLB_COMMS_UNREACHABLE();
  }

  static std::exception_ptr extract_error(type&) { DLB_COMMS_UNREACHABLE(); }
};
/// @endcond
...
class [[nodiscard]] async_result {
 ...
 public:
  ...
  using result_state = result_state_type<traits::optimize_for_size, T, traits>;
  /// @endcond
   ...
   */
  async_result(const low_level_solver_ptr<T, traits>& on_result,
               async_result_tags::low_level_tag)
      : state_(make_solver_state(on_result)) {}
   ...
   * @endcode
   */
  static async_result_with_solver<T, traits> make() {
    return async_result_with_solver<T, traits>{};
  }
   ...
   */
  template <typename TT = T,
            typename X = std::enable_if_t<!std::is_same<TT, void>::value &&
                                              std::is_convertible<TT, T>::value,
                                          T>>
  async_result(TT&& val,
               async_result_tags::resolved_tag = async_result_tags::resolved)
      : state_(make_value_state_nv(std::forward<TT>(val))) {}
  ...
  async_result() : state_(make_value_state_v()) {}
  ...
  template <
      typename TT = T,
      typename X =
          std::enable_if_t<std::is_same<TT, std::exception_ptr>::value, T>>
  async_result(TT&& e,
               async_result_tags::failed_tag = async_result_tags::failed)
      : state_(make_error_state(std::forward<TT>(e))) {}
  ...
  auto then(U&& cb, err_cb_type&& local_err_cb = {}) && {
    if constexpr (traits::optimize_for_size) {
      using cb_ret = typename detail::cb_type_helper<T, U>::cb_ret_type;
      using cb_arg = T;
      if constexpr (std::is_same_v<void, cb_arg>) {
        return std::move(*this).then_internal(
            typename traits::template callback_type<cb_ret(void)>{
                std::forward<U>(cb)},
            std::move(local_err_cb));
      } else {
        return std::move(*this).then_internal(
            typename traits::template callback_type<cb_ret(cb_arg&&)>{
                std::forward<U>(cb)},
            std::move(local_err_cb));
      }
    } else {
      return std::move(*this).then_internal(std::forward<U>(cb),
                                            std::move(local_err_cb));
    }
  }
   ...
   */
  auto consume_errors(
      typename traits::template callback_type<T(std::exception_ptr&&)>&&
          err_cb) && -> async_result<T, traits> {
    ...
    switch (holds()) {
      ...
      case async_result_holds::solver: {
        ...
        auto chained_slv = std::make_shared<low_level_solver<T, traits>>(
            async_result_tags::intentional_usage);
        ...
        bool already_has_error = end_propagation_slv->set_error_callback(
            [chained_slv = chained_slv.get(),
             err_cb{std::move(err_cb)}](std::exception_ptr&& e) mutable {
              try {
        ...
        if (!already_has_error) {
          already_has_result = end_propagation_slv->set_callback(
              solver_lambda_helpers<T, traits>::chain_value(chained_slv.get()));
        }

        if (!already_has_result) {
          end_propagation_slv->set_propagate_error_callback(
              typed_lambda_helpers<std::exception_ptr>::empty());
        }
  ...
  void on_error(err_cb_type&& err_cb) && {
    ...
    switch (holds()) {
      ...
      case async_result_holds::solver: {
        ...
        bool already_failed =
            old_solver->set_propagate_error_callback(std::move(err_cb));
        if (!already_failed) {
  ...
  explicit operator bool() const {
    return holds() != async_result_holds::nothing;
  }

 private:
  using state_t = typename result_state::type;

  static state_t make_solver_state(
      const low_level_solver_ptr<T, traits>& on_result) {
    if constexpr (traits::optimize_for_size) {
      return on_result;
    } else {
      return state_t{std::in_place_index_t<async_result_holds::solver>{},
                     on_result};
    }
  }
  ...
  template <typename TT = T,
            typename X = std::enable_if_t<!std::is_same<TT, void>::value &&
                                              std::is_convertible<TT, T>::value,
                                          T>>
  static state_t make_value_state_nv(TT&& val) {
    if constexpr (traits::optimize_for_size) {
      auto slv = std::make_shared<class low_level_solver<T, traits>>(
          async_result_tags::intentional_usage);
      slv->resolve(std::forward<TT>(val));
      return slv;
    } else {
      return state_t{std::in_place_index_t<async_result_holds::value>{},
                     std::forward<TT>(val)};
    }
  }
  ...
  template <typename TT = T,
            typename X = std::enable_if_t<std::is_same_v<TT, void>, T>>
  static state_t make_value_state_v() {
    if constexpr (traits::optimize_for_size) {
      auto slv = std::make_shared<class low_level_solver<T, traits>>(
          async_result_tags::intentional_usage);
      slv->resolve();
  ...
  static state_t make_error_state(std::exception_ptr&& e) {
    if constexpr (traits::optimize_for_size) {
      auto slv = std::make_shared<class low_level_solver<T, traits>>(
          async_result_tags::intentional_usage);
      slv->fail(std::move(e));
      ...
      return state_t{std::in_place_index_t<async_result_holds::error>{},
                     std::move(e)};
    }
  }

  template <typename U>
  auto then_internal(U&& cb, err_cb_type&& local_err_cb = {}) && ->
      typename detail::return_type_helper<
          typename detail::cb_type_helper<T, U>::cb_ret_type,
          traits>::async_result_type {
    assert(holds() != async_result_holds::nothing);
    switch (holds()) {
      case async_result_holds::value: {
        return invoke_cb_and_consume_errors(std::forward<U>(cb),
                                            extract_value());
      }
      case async_result_holds::error: {
      ...
      case async_result_holds::solver: {
        using ret_type_info = detail::return_type_helper<
            typename detail::cb_type_helper<T, U>::cb_ret_type, traits>;

        auto my_solver = extract_solver();
        solver_synchronisation_guard<traits> lock(my_solver->base());
        auto chain_result =
            set_local_error_callback_and_chain<typename ret_type_info::type>(
                std::move(local_err_cb), my_solver);

        bool already_has_result = false;
        if (!chain_result.already_has_error) {
          already_has_result =
              set_result_callback<typename ret_type_info::type>(
                  my_solver, std::forward<U>(cb));
        }
        ...
        return {std::move(chain_result.chained_slv),
                async_result_tags::intentional_usage};
      }
    }
    DLB_COMMS_UNREACHABLE();
  ...
  size_t holds() const { return result_state::holds(state_); }
  void reset() { result_state::reset(state_); }
      ...
      if constexpr (std::is_same_v<
        ...
        return {invoke_cb(std::forward<U>(cb), std::forward<Val>(value)),
                async_result_tags::resolved};
                                           ...
                                           typename detail::cb_type_helper<
                                               T, U>::cb_ret_type>) {
        return invoke_cb(std::forward<U>(cb), std::forward<Val>(value));
      } else {
  ...
  low_level_solver_ptr<T, traits> extract_solver() {
    return result_state::extract_solver(state_);
  }

  typename variant_value<T>::type extract_value() {
    return result_state::extract_value(state_);
  }

  std::exception_ptr extract_error() {
    return result_state::extract_error(state_);
  }

  static bool set_local_error_callback(
      err_cb_type&& local_err_cb,
      low_level_solver_ptr<T, traits>& my_solver) {
    bool already_has_error = false;
    if (local_err_cb) {
      already_has_error =
          my_solver->set_error_callback(std::move(local_err_cb));
    }
  ...
  template <typename U>
  static auto set_local_error_callback_and_chain(
      err_cb_type&& local_err_cb,
      low_level_solver_ptr<T, traits>& my_solver) {
    auto already_has_error =
        set_local_error_callback(std::move(local_err_cb), my_solver);

    auto slv = std::make_shared<low_level_solver<U, traits>>(
        async_result_tags::intentional_usage);
    my_solver->set_chained_solver(slv);
    return set_local_error_callback_and_chain_result<U>{std::move(slv),
                                                        already_has_error};
  }

  template <typename SlvType, typename CbType>
  static void propagate_nested_callback_value(
      low_level_solver<T, traits>* my_solver,
      CbType& cb) {
    auto nested_result = detail::wrap_as_solver<decltype(cb), decltype(cb()),
                                                traits>::invoke_and_wrap(cb);
    auto* slv = static_cast<low_level_solver<SlvType, traits>*>(
        my_solver->get_chained_solver().get());
    switch (nested_result.holds()) {
      case async_result_holds::value: {
        auto val = nested_result.extract_value();
        if constexpr (std::is_same_v<variant_value<void>::type,
                                     decltype(val)>) {
          slv->resolve();
        } else {
      ...
      case async_result_holds::solver: {
        // that solver obviously does not have any callbacks attached, and is
        // not chained:
  ...
  static std::enable_if_t<!std::is_same_v<void, TT>, bool> set_result_callback(
      ...
      U&& cb) {
    return my_solver->set_callback(
        [cb{std::forward<U>(cb)},
         my_solver = my_solver.get()](TT&& val) mutable {
          auto wrapper_cb = [val{std::forward<TT>(val)}, &cb]() mutable {
  ...
  static std::enable_if_t<std::is_same_v<void, TT>, bool> set_result_callback(
      ...
      U&& cb) {
    return my_solver->set_callback(
        [cb{std::forward<U>(cb)}, my_solver = my_solver.get()]() mutable {
          propagate_nested_callback_value<SlvType>(my_solver, cb);
        });
  }

  template <typename U>
  static void set_propagate_error_callback(
      const low_level_solver_ptr<T, traits>& from,
      const low_level_solver_ptr<U, traits>& to) {
    from->set_propagate_error_callback(
        [to = to.get()](std::exception_ptr&& err) {
          to->fail(std::move(err));
        });
  }
 ...
 *
 * This function is a helper function, using the core async_result API for
 * constructing a new result, being a product of several transformations of the
 * original result: discarding its produced value, returning empty
 * std::exception_ptr if no errors happened, or consuming errors (in terms of
 * using async_result::consume_errors() method) and returning non-empty
 * std::exception_ptr if errors happened.
 *
 * Use this method if a potential error produced by asynchronous operations
 * needs handling asynchronously. Keep in mind, that the product of this method
 * is an asynchronous result which is always resolved with success, and the
 * original error can be extracted from the std::exception_ptr passed down the
 * chain of operations in the value callback.
 *
 * @param res the result to convert into asynchronously produced exception
 * pointer.
 * @return async_result holding the std::exception, which always resolves with
 * success.
 */
template <typename T, typename traits>
inline async_result<std::exception_ptr, traits> discard_result_and_get_error(
    async_result<T, traits>&& res) {
  return async_result_helper<traits>::discard_result_and_get_error(
      std::move(res));
}
