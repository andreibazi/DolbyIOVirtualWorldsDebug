...
struct DLB_COMMS_EXPORT notification_subscription {
   ...
   */
  notification_subscription(const std::string& conference_alias,
...
class DLB_COMMS_EXPORT session {
 public:
  ...
  struct user_info {
    ...
    std::string
        externalId; /**< The external unique identifier that the customer's
    ...
    std::optional<std::string>
        participant_id{}; /**< The unique ID for the participant opening
  ...
  /**
   * @brief Updates the metadata of a logged in user.
   *
   * @param identification Information about the participant who tries to open a
   * session. Only the name and avatarUrl are updated, other fields of the user
   * identification are ignored.
   *
   * @returns The result object producing the new user_info asynchronously.
   *
   * \code{.cpp}
   ...
   * @returns The result object producing the operation status asynchronously.
   */
  virtual async_result<void> subscribe(
      const std::vector<notification_subscription>& subscriptions) = 0;
   ...
   * @returns The result object producing the operation status asynchronously.
   */
  virtual async_result<void> unsubscribe(
      const std::vector<notification_subscription>& subscriptions) = 0;
   ...
   */
  virtual async_result<event_handler_id> add_event_handler(
      event_handler<token_expired_event>&& callback) = 0;
};
}  // namespace services
}  // namespace dolbyio::comms
