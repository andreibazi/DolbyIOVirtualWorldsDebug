...
struct DLB_COMMS_EXPORT participant_info {
  ...
  struct info {
    ...
    std::optional<std::string>
        external_id{}; /**< The external unique identifier that the customer's
    ...
    std::optional<std::string>
        avatar_url{}; /**< The URL of the participant's avatar. */
  };
  ...
  std::optional<participant_status>
      status{}; /**< The current status of the participant. */
  info info;    /**< Additional information about the participant. */
  std::optional<bool>
      is_sending_audio{}; /**< A boolean that informs whether
                  the participant is sending audio into conference. */
  std::optional<bool>
      audible_locally{}; /**< Indicates whether a remote participant is audible
                            locally. This will always be false for the local
                            participant. */

  bool is_in_conference() const noexcept {
    return status.has_value() && status.value() == participant_status::on_air;
  }
  bool operator<(const participant_info& other) const noexcept {
...
using local_participant = participant_info;
using remote_participant = participant_info;
