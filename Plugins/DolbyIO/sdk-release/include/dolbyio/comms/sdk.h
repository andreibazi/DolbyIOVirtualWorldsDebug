...
#include <string>
    ...
    !defined(DOLBYIO_COMMS_SUPPRESS_APPLE_NO_RTTI_WARNING)
...
class DLB_COMMS_EXPORT sdk {
 public:
   ...
   *
   * This method causes the global operators new and delete in the SDK to use
   * the application-provided allocator. If the application wishes to override
   * the SDK's memory allocator, it must do so prior to calling any other SDK
   * APIs, otherwise there is a risk of overriding memory deallocation functions
   * and using the overriden functions to free memory that has been allocated
   * using the default memory allocator. The SDK restores the default allocator
   * upon application exit (not the SDK destruction) to allow freeing memory
   * allocated in global objects using the system allocator.
   *
   * If the allocator has already been overridden, and the new one uses
   * different memory management functions, this method will throw. If the
   * allocator does not contain all four memory allocation functions, this
   * method will also throw. The application should not attempt to restore the
   * default allocator before shutdown; the SDK does it automatically.
   *
  ...
  struct log_settings {
    ...
    /**
     * @brief Log level for DVC logs.
     * It is recommended to keep the DVC log level at OFF, ERROR or
     * WARNING to avoid spam and only enable more detailed logs when necessary.
     * Default is OFF.
     */
    ...
    /**
     * @brief Suppress the logs to stdout. This parameter will only be taken
     * into account if the log directory is set. In which case logs will only be
     * passed to the file in directory and nothing on stdout.
     */
  ...
  static std::unique_ptr<sdk> create(
      ...
      std::function<void(std::unique_ptr<refresh_token>)>&&
  ...
  /**
   * @brief Component specific data returned by the SDK when the components
   * is registered.
   */
  struct component_data {
    /**
     * @brief Shared pointer to the logger instance created for the component
     * when it is registered with the SDK. This can be used to log using the SDK
     * logger and thus log to the same file as the SDK.
     */
   ...
   *
   * The component name and version are free-form strings, which will be added
   * to the Dolby.io telemetry reports. If a component with given name has
   * already been registered with a different version, this method fails and
   * returns error information asynchronously.
   *
   * @param name the name of the component
   * @param version the version of the component
   *
   * @return The result object producing the success or failure information
   * asynchronously.
   *
   * \code{.cpp}
   ...
   * telemetry" << std::endl; }); \endcode
   */
  virtual async_result<component_data> register_component_version(
  ...
  virtual async_result<event_handler_id> add_event_handler(
      event_handler<signaling_channel_exception>&& callback) = 0;
   ...
   */
  virtual async_result<event_handler_id> add_event_handler(
      event_handler<invalid_token_exception>&& callback) = 0;
};
