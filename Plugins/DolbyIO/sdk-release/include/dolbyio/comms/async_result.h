...
using low_level_solver_ptr =
    detail::low_level_solver_ptr<T, thread_safe_solver_traits>;

/**
 * @brief Partial specialization of the detail::solver template class, ensuring
 * that the operations are thread-safe.
 * @ingroup async_ops
 */
template <typename T>
...
/**
 * @brief Partial specialization of the detail::async_result_with_solver
 * template class, ensuring that the operations are thread-safe.
 * @ingroup async_ops
 */
template <typename T>
using async_result_with_solver =
    dolbyio::comms::detail::async_result_with_solver<T,
                                                     thread_safe_solver_traits>;

/**
 * @deprecated Since 2.3. For optimal performance blocking until async_result is
 * resolved is not recommended. However, if needed it is the applications
 * responsibility to wait for result of asynchronous operation to become
 * available.
 *
...
T wait(async_result<T>&& asyncop) {
  ...
  if constexpr (std::is_same_v<void, T>) {
    std::move(asyncop)
        .then([prom]() { prom->set_value(); })
        .on_error([prom](auto&& e) { prom->set_exception(std::move(e)); });
  } else {
    std::move(asyncop)
        .then([prom](T&& val) { prom->set_value(std::move(val)); })
        .on_error([prom](auto&& e) { prom->set_exception(std::move(e)); });
  }
  return fut.get();
}
