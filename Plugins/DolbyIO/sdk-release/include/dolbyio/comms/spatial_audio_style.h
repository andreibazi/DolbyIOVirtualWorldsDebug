...
enum class DLB_COMMS_EXPORT spatial_audio_style {
  ...
  shared,     /**< Spatial audio for shared scenes, The shared option sets the
                 ...
                 participants are calculated by the     Dolby.io server. This way,
                 the     spatial scene is shared by all participants, so     that each
                 client     can set its own position and participate in the shared
                 scene.     This     approach simplifies communication between the
                 client and the server     and     decreases network traffic. */
};
