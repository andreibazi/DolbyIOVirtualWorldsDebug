 ...
 */
class DLB_COMMS_ADDON_EXPORT recorder
 ...
 public:
   ...
   * stored in Matroska.
   *
   * @note If the configuration is set to ENCODED_OPTIMIZED, then the video
   * frames are not decoded after they are dumped. In this case if you call the
   * dolbyio::comms::services::remote_video service::set_video_sink method, the
   * frames will never reach the set sink. If you would like to get decoded
   * frames as well as dump encoded frames to a file make sure to set the
   * configuration to ENCODED format.
   */
  enum class video_recording_config {
    ...
    ENCODED_OPTIMIZED /**< Enables capturing of the video frames that are
                         encoded via the VP8 or H264 codec. Only the first frame
                         of the incoming video track will be decoded. */
  };
   ...
   */
  recorder(dolbyio::comms::sdk& sdk,
