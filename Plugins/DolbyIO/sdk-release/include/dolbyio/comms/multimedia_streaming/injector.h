...
struct DLB_COMMS_ADDON_EXPORT media_injection_status {
  ...
  state state_{
      STOPPED}; /**<  Holds the value of current state for single media. */
 ...
 * @ingroup default_injector
 */
class DLB_COMMS_ADDON_EXPORT injector
    : public video_frame_handler,
      public video_source,
      public audio_source,
      public std::enable_shared_from_this<injector> {
 public:
  using media_injection_status_cb =
      std::function<void(const media_injection_status&)>;
   ...
   *
   * The application may use this callback to start/stop injection, but can be
   * configured manually, regardless of the injector's video sink status.
   *
   * @param cb
   */
 ...
 private:
  ...
  void set_sink(const std::shared_ptr<video_sink>& sink,
                const video_source::config& config) override;
...
class DLB_COMMS_ADDON_EXPORT injector_passthrough : public injector {
 public:
   ...
   * @retval false On failure to push frame.
   */
  bool inject_video_frame(const video_frame& frame) override;
};
...
class DLB_COMMS_ADDON_EXPORT injector_paced : public injector {
 public:
   ...
   * @retval false On failure to push frame.
   */
  bool inject_video_frame(const dolbyio::comms::video_frame& frame) override;

  /**
   * @brief Sets the video frame rate in frames per second for injecting video
   * frames into the conference as a numerator/denominator pair. The pacing
   * injector provides frames to WebRTC at the interval specified by this
   * method, provided the frames are available on the queue.
   *
   ...
   * options for stopping injection; the default option, which is considered
   * forceful stoppage, stops the video pacing thread immediately. Frames that
   * are on the queue when a force stop is invoked remain. The second option,
   * which is considered graceful stoppage allows the caller to wait until all
   * the frames on the queue are popped off by the video pacing thread, then
   * stops the thread.
   *
   ...
   * frames from the queue and injecting them into the conference. There are two
   * options for stopping injection; the default option, which is considered
   * forceful stoppage, stops the audio pacing thread immediately. Frames that
   * are on the queue when a force stop is invoked remain. The second option,
   * which is considered graceful stoppage allows the caller to wait until all
   * the frames on the queue are popped off by the audio pacing thread, then
   * stops the thread.
   *
   * @param force Boolean indicating whether to forcefully shutdown the
