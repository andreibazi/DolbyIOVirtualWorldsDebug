...
struct spatial_environment {
   ...
   * forward and up.
   */
  constexpr spatial_environment(const spatial_scale& scale,
  ...
  spatial_scale
      scale; /**< A scale that defines how to convert units from the
                coordinate system of an application (pixels or centimeters)
                into meters used by the spatial audio coordinate system. */
  spatial_position
      forward;            /**< A vector describing the direction the application
  ...
  spatial_position right; /**< A vector describing the direction the application
                             considers as right. The value can be either +1, 0,
                             or -1 and must be orthogonal to forward and up. */
};
...
class DLB_COMMS_EXPORT spatial_audio_batch_update {
 public:
   ...
   */
  void set_spatial_position(const std::string& participant_id,
                            const spatial_position& position);
